    ...
    public partial class MainWindow : Window
    {
        ...
        private void SetUser(User user, bool wasAlreadyEnteringThroughThisApp)
        {
            ...
            if (user.SongList.Count > 0)
            {
                ...
                Player1.SetTimer(_updateInterval, new EventHandler(timerUpdate_Tick));
                Player1.Play();
            }
            else
            {
        ...
        private void rectangleProgressBarMain_MouseDown(object sender, MouseButtonEventArgs e)
        {
            if (_CurrentUser == null || _CurrentSong < 0)
                return;
            long len = Bass.BASS_ChannelGetLength(Player1.GetStream());
            double multiplyer = e.GetPosition(rectangleProgressBarMain).X / rectangleProgressBarMain.Width;
            long pos = (long)multiplyer * len;
            Bass.BASS_ChannelSetPosition(Player1.GetStream(), pos);
        }
        #endregion
    }
}
